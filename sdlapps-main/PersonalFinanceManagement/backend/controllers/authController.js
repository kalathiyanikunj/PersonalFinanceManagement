...
const saveData = async (req, res) => {
    try {
        ...
        res.json({ id: saveData.id, Title: saveData.Title, Description: saveData.Description, Date: saveData.Date, token: generateToken(updatedUser.id) });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};

module.exports = { registerUser, loginUser, updateUserProfile, getProfile, saveData };
